...
18. 前端支持 iframe / web-view 嵌入模式，并读取 `studentId`、`studentName`、`assignmentId`、`token`。
19. 已提供 `database-schema.sql` 作为正式 MySQL 数据库结构草案，并新增 MySQL 存储适配。
...
已完成本轮优先项：真实二维码、Layout JSON、拍照上传批改、客观题自动判分、小程序参数适配、MySQL 存储适配。
